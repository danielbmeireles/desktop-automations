...
# Backup Brewfile generated by updater script
if [ -f "$HOME_DIR/Brewfile" ]; then
    log "Backing up Brewfile from $HOME_DIR/Brewfile"
    mkdir -p "$BACKUP_DIR/brew"
    cp -f "$HOME_DIR/Brewfile" "$BACKUP_DIR/brew/Brewfile" >> "$LOG_FILE" 2>&1 \
        || log "Warning: failed to copy Brewfile"
else
    log "Brewfile not found at $HOME_DIR/Brewfile; skipping"
fi
